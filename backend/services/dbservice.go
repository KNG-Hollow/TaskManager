...
func AddAccount(account models.Account) (bool, error) {
	...
	if err != nil {
		stat = false
		log.Panicln("Failed To Hash Password:", err)
	}
	...
	insert, err := db.Exec(
		...
		account.ID, account.Name, account.Username, password, boolToBit(account.Admin), boolToBit(account.Active),
	)
...
func UpdateAccount(id int64, newData models.Account) (bool, error) {
	...
	password, err := hashPassword(newData.Password)
	if err != nil {
		stat = false
		log.Panicln("Failed To Hash Password:", err)
	}
	...
	update, err := db.Exec(
		...
		newData.Name, newData.Username, password, newData.Admin, newData.Active, id,
	)
